...
namespace GraphQLServer
{
    ...
    public class Query
    {
        ...
        public async Task<User> GetUserByToken()
        {
            ...
            var _dataBaseConnectionToken = await _dataBaseConnection.AuthorizationTokens.FirstOrDefaultAsync(q => q.c_token == token);

            if (_dataBaseConnectionToken == null)
                throw new ArgumentException("TOKEN_NOT_FOUND_PROBLEM");

            UserData userData = await _dataBaseConnection.Users.FirstOrDefaultAsync(q => q.f_authorizationtoken == _dataBaseConnectionToken.id);
            if (userData == null)
            ...
            User user = new User
            {
                ...
                FRole = await _dataBaseConnection.Roles.FirstOrDefaultAsync(q => q.id == userData.f_role),
            };
            return user;
        ...
        public async Task<string> LoginUser(string login, string password)
        {
            ...
            var token = new JwtSecurityTokenHandler().WriteToken(
                Helpers.GenerateNewToken(user.id.ToString()));

            var authorizationToken =
                await _dataBaseConnection.AuthorizationTokens.FirstOrDefaultAsync(q => q.id == user.f_authorizationtoken);
